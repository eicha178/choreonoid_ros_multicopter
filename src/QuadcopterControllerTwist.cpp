...
class QuadcopterControllerTwist : public SimpleController
{
public:
...
bool QuadcopterControllerTwist::configure(SimpleControllerConfig* config)
{
...
bool QuadcopterControllerTwist::initialize(SimpleControllerIO* io)
{
    ioBody = io->body();
    os = &io->os();
    ...
    joystickSubscriber = node->subscribe("joy", 1, &QuadcopterControllerTwist::joystickCallback, this);
...
Vector4 QuadcopterControllerTwist::getZRPY()
{
...
Vector2 QuadcopterControllerTwist::getXY()
{
    auto p = ioBody->rootLink()->translation();
    return Vector2(p.x(), p.y());
}

void QuadcopterControllerTwist::joystickCallback(const sensor_msgs::Joy& msg)
{
    std::lock_guard<std::mutex> lock(joystickMutex);
    latestJoystickState = msg;
}

bool QuadcopterControllerTwist::control()
{
...
void QuadcopterControllerTwist::stop()
{
    joystickSubscriber.shutdown();
}

CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(QuadcopterControllerTwist)
